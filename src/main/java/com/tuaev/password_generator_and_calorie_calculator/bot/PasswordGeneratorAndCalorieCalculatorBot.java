...
import com.tuaev.password_generator_and_calorie_calculator.enums.Floor;
...
public class PasswordGeneratorAndCalorieCalculatorBot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        ...
        isCommand(message, userId);

        checkOnStateUserById(userId, message);
    }

    private void checkOnStateUserById(String userId, Message message) {
        if (stateUserById.get(userId) == Commands.CALORIES) {
            ...
            if (count == questionsCalories.length) {
                ...
                String floor = responsesUserOnQuestionsCalories.get(userId).get(3).get(QuestionsCalories.FLOOR);
                ...
                String result = String.format("""
                        ...
                        """, calorieCalculator(weight, height, age, floor, activity));
            ...
            if (count < questionsCalories.length) {
                if (questionsCalories[count] == QuestionsCalories.FLOOR) {
                    ReplyKeyboardMarkup markup = getReplyKeyboardMarkupFloor();
                    sendMessage(questionsCalories[count].getText(), userId, markup);
                    count++;
                    ...
                    ReplyKeyboardMarkup markup = getReplyKeyboardMarkupActivity();
                    sendMessage(text, userId, markup);
                    count++;
                    iteratorUserById.replace(userId, count);
                } else {
                    sendMessage(questionsCalories[count].getText(), userId, deleteKeyboard());
                    count++;
                    iteratorUserById.replace(userId, count);
                }
            }
    ...
    private void isCommand(Message message, String userId) {
        if (message.isCommand() && message.getText().equals(Commands.START.getText())) {
            ...
            sendMessage(Commands.CALORIES.getInfo(), userId);
    ...
    private ReplyKeyboardMarkup getReplyKeyboardMarkupFloor() {
        ReplyKeyboardMarkup markup = new ReplyKeyboardMarkup();
        List<KeyboardRow> rows = new ArrayList<>();
        KeyboardRow row = new KeyboardRow();
        KeyboardButton button1 = new KeyboardButton(Floor.MAN.getText());
        KeyboardButton button2 = new KeyboardButton(Floor.WOMAN.getText());
        row.add(button1);
        row.add(button2);
        rows.add(row);
        markup.setKeyboard(rows);
    ...
    private int calorieCalculator(int weight, int height, int age, String floor, String activity) {
        ...
        if (floor.equals(Floor.MAN.getText())) {
    ...
    private void getError(TelegramApiException e) {
